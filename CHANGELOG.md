...
- 新增 95 分发布门槛：A11 QA 低于 95/100 的生产提示词不得交付用户。
- 新增“骨架-骨骼审计”：连续性模式、时间码功能、相机路径、表演动作、空间调度、道具机制、声音边界和终帧状态必须逐项命名。
- 强化 D 线一镜到底判定，防止把时间码动作 beat 误判为 SH 硬切镜头。
- 重新定义 B 线为长片级“资产 + 故事板 + Seedance 2.0 全能参考”生产线，用资产保证统一性、故事板控制叙事镜头、Seedance 融合执行。
- 新增 B 线长片前端拆解门禁：全片结构、逐段 DNA ledger、跨段连续性圣经、遗漏审核和 `B_FRONTEND_SIGNOFF` 通过后，才允许进入资产、故事板和 Seedance。
