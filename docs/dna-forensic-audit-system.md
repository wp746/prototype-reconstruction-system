...
- 出词前必须执行 95 分发布门槛。低于 95/100 的生产提示词不得交给用户，必须回到漏项维度修正。
...
不能只确认大骨架，还必须确认每个骨架下面的骨骼：

| Skeleton | Bones That Must Be Named |
|---|---|
| 连续性模式 | 一镜到底、伪一镜到底、硬切、多段拼接、固定机位、手持 POV、屏幕录制；时间码是动作 beat 还是切点 |
| 镜头/Beat | 每个镜头或 beat 的功能、起止、主体位置、相机位置、动作结果 |
| 运镜 | 操作者位置、路径、距离变化、速度曲线、焦点变化、抖动/稳定逻辑 |
| 表演 | 眼神、嘴型、手部、身体角度、情绪变化、台词语气 |
| 空间 | 起点、终点、入口/出口、角色区、道具锚点、前中后景 |
| 道具/机制 | 被触碰物、触发方式、问题点、处理动作、终态 |
| 声音 | 画内声、台词/旁白、静默、BGM 后期边界、声音是否驱动动作或切点 |
| 终帧 | 最后可读姿态、情绪状态、物体状态、镜头停留时间 |

任何骨骼未命名，`DNA_SIGNOFF` 不能通过。
...
任一关键项为 `no`，或 `qa_score < 95`，不得进入资产、故事板、Seedance。
