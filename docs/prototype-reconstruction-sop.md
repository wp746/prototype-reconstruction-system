...
- 选择哪条 Seedance 交付支线：A 线资产直出、B 线黑白手稿故事板参考、C 线无资产文本直出，或 D 线已有提示词重构。
...
- C 线不写不存在的资产编号、模块标签或 `@图片` 引用。
- D 线必须先输出提示词 DNA、变量/不变量和至少 3 个重构方向；用户选择方向后，最终提示词只描述新片本身，不写原始提示词或改写过程。
...
- D 线已有提示词任务包含 `prompt_dna_ledger`、`variable_map`、`invariant_map` 和三方向选择记录。
