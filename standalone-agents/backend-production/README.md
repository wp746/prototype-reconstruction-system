...
- 风格合同 `STYLE_CONTRACT`：最终统一风格、媒介、真实度、材质、光线、镜头、色彩和禁用风格。

如果前端没有给出这些字段，本 agent 不能假装已经完成前端拆解；必须要求前端补齐。风格尤其是硬门槛：无法从前端物料可靠判断时，不得进入资产提示词生产。
...
- 角色、场景、道具资产提示词必须全部包含 `STYLE_CONTRACT_LOCK` 和 `STYLE_NEGATIVE`。
