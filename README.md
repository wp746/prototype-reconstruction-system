...
B 线是长片级“资产 + 故事板 + Seedance 2.0 全能参考”生产线：资产锁角色、场景、道具和风格统一性，故事板锁镜头顺序、构图、动作方向、切点和叙事节拍，Seedance 负责融合执行。C 线使用紧凑因果锁，通常控制在用户要求的字数内，不混入 A/B 线的资产板和故事板负面词。D 线先把已有提示词当作 15 秒原型拆解，输出变量/不变量、可替换层和至少 3 个重构方向，再按用户选择改写。
...
四支线全局方法见 [复刻重构四支线全局规则](docs/global-three-branch-remake-system.md)。A 线遵循“身份资产 -> 环境资产 -> 道具/机制资产 -> 执行态资产 -> 最终 payoff 资产”的资产职责隔离模型；B 线是长片主生产线，先做资产包和分段故事板，再让 Seedance 2.0 全能参考同时读取资产和干净故事板；C 线为无资产纯文本直出，使用紧凑因果锁而不是资产或故事板引用；D 线为提示词 DNA 重构，必须先拆变量和不变量，再给三个方向供用户选择。若单条 12-15 秒生成无法同时保住快节奏和复杂状态，系统应主动建议拆段生成再剪。
