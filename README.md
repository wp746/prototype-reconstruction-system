...
复刻 / 重构进入生产时，系统按输入条件和控制目标选择 A/B/C/D 四条支线：
...
| B 线：资产+故事板长片控制 | 用户要做长一点的复刻/重构片，需要资产、故事板和 Seedance 2.0 全能参考协作 | 资产包 + 分段故事板 + 干净黑白控制帧 + Seedance 全能参考提示词 | 用资产保统一性，用故事板控叙事镜头，用 Seedance 融合执行 | 故事板污染身份，多段状态断裂，资产/故事板优先级混乱 |
| C 线：纯文本直出 | 用户没有资产和故事板，只要一条 Seedance 提示词快速验证 | 无上传图，只用文本 | 快速验证剧情结构、镜头节奏和因果链 | 身份、道具、细节一致性弱，复杂状态变化不稳定 |
| D 线：提示词重构 | 用户拿到一段已有提示词，希望基于这段提示词复刻或重构 | 原始提示词、目标时长、目标模型、可选替换需求 | 拆出提示词 DNA，替换风格/元素/角色/场景/台词/故事等变量 | 直接改写导致结构断裂，变量和不变量混淆 |

四条支线都必须先完成后台 DNA 拆解，最终模型提示词不得出现“参考片、原片、学习、复制、原始提示词、照着改”等源片/源提示词过程语言。
...
B 线是长片级“前端 DNA 拆解 + 资产 + 故事板 + Seedance 2.0 全能参考”生产线：前端先做全片结构、逐段 DNA 和跨段连续性圣经，过 `B_FRONTEND_SIGNOFF` 后，资产锁角色、场景、道具和风格统一性，故事板锁镜头顺序、构图、动作方向、切点和叙事节拍，Seedance 负责融合执行。C 线使用紧凑因果锁，通常控制在用户要求的字数内，不混入 A/B 线的资产板和故事板负面词。D 线先把已有提示词当作 15 秒原型拆解，输出变量/不变量、可替换层和至少 3 个重构方向，再按用户选择改写。

完整规则见 [复刻重构四支线全局规则](docs/global-three-branch-remake-system.md) 和 [Seedance 2.0 四支线交付协议](docs/seedance-two-branch-delivery-protocol.md)。
...
目标：先做 DNA 拆解与遗漏审核，再按 A/B/C/D 支线输出可生产提示词。
```
...
这个项目对外是一个可安装 Skill，对内是一个 `Master Agent + A0-A11 + D-line Prompt DNA Agent` 的多智能体协作系统：
...
-> A10 后端交接 / A11 QA
-> D 线提示词 DNA：已有提示词变量/不变量拆解与重构
```

对外分享时只需要安装仓库或 `SKILL.md`；真正执行任务时，Master Agent 会按素材条件和目标自动选择 A/B/C/D 支线，并只加载必要的子 Agent。

推荐先看：

- [Master Agent 执行入口](agents/master-agent.md)
- [D 线提示词 DNA Agent](agents/d-line-prompt-dna.md)
...
- [复刻重构四支线全局规则](docs/global-three-branch-remake-system.md)
- [Seedance 2.0 四支线交付协议](docs/seedance-two-branch-delivery-protocol.md)
- [B 线长片资产+故事板+Seedance 协议](docs/branch-b-longform-asset-storyboard-seedance.md)
...
Seedance 2.0 后期交付分四条支线：A 线为资产直出 Seedance，不上传故事板，优先保护用户指定角色、场景和道具；B 线为资产+故事板+Seedance 2.0 全能参考，适合长一点的复刻/重构片，用资产保证角色/场景/道具/风格统一，用故事板控制镜头叙事和段落节奏；C 线为无资产文本直出 Seedance，适用于用户没有资产、也没有要求资产/故事板、只要一条 Seedance 2.0 提示词的快速复刻或重构；D 线为已有提示词重构，适用于用户拿到一段 12-15 秒提示词，希望替换风格、元素、角色、场景、台词或故事变量。A/B 线必须使用明确引用写法，例如 `参考 A01 / M03 FACE CLOSE-UP 锁定角色脸部`、`参考 A02 / V04 CAMERA A 锁定场景机位`、`参考 A03 / P06 HAND LOGIC 锁定道具持握`；C/D 线没有资产引用时，必须用短而稳定的文本锚点锁角色、场景、道具和风格。15 秒以内短片必须写成一条完整连续视频，逐镜 timing 只是片内切点，不是拆成多条视频。

四支线全局方法见 [复刻重构四支线全局规则](docs/global-three-branch-remake-system.md)。A 线遵循“身份资产 -> 环境资产 -> 道具/机制资产 -> 执行态资产 -> 最终 payoff 资产”的资产职责隔离模型；B 线是长片主生产线，先做前端全片结构拆解、逐段 DNA ledger、跨段连续性圣经和 `B_FRONTEND_SIGNOFF`，再做资产包和分段故事板，让 Seedance 2.0 全能参考同时读取资产和干净故事板；C 线为无资产纯文本直出，使用紧凑因果锁而不是资产或故事板引用；D 线为提示词 DNA 重构，必须先拆变量和不变量，再给三个方向供用户选择。若单条 12-15 秒生成无法同时保住快节奏和复杂状态，系统应主动建议拆段生成再剪。
...
不管使用逐镜协议还是四阶兜底协议，都必须禁止字幕、水印、乱码、故事板边框和随机文字进入画面。A/B 线必须明确引用资产图号和模块标签；C 线没有资产图时必须用稳定文本锚点替代资产引用；D 线必须先从原始提示词中提取稳定文本锚点，再按用户选择替换变量。参考片或原始提示词里的文字若承担台词、咒语或旁白功能，只重写为新片原创声音/表演节奏，不做画面字幕。
