...
D 线必须先判断连续性模式。原提示词如果写第一人称手持、镜头随主角移动、操作者呼吸感、自动对焦搜索、自动白平衡漂移、自然跟随等，一般是不剪切的一镜到底。时间码只是动作 beat，不要改成 SH 硬切。

D 线最终提示词骨架：

```text
[VIDEO TASK]
生成一条完整连续的 [时长] 秒视频，[画幅]，[fps]。[连续性模式]。如果是第一人称一镜到底手持跟拍，以下 BEAT01-BEAT[N] 是同一镜头内的连续动作段落，不是硬切镜头，不要跳切、不要换机位、不要镜头瞬移。
...
[BEAT TIMING]
BEAT01 / [start-end] / [continuous handheld position change] / [single action beat + visible result]
BEAT02 / ...
