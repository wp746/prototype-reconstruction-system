...
description: Use when the user provides a video clip, short film, link, screenshots, production assets, or an existing video-generation prompt and wants AIGC reference remake, prototype reconstruction, video/prompt DNA deconstruction, Image2 asset/storyboard prompts, or Seedance 2.0 A/B/C/D branch prompts. The skill performs DNA-level analysis, chooses asset-based, storyboard-controlled, text-only, or prompt-to-remake delivery, and outputs source-free production prompts with QA.
---
...
This repository is installed as one Skill, but it runs as one Master Agent coordinating A0-A11 specialist agents plus the D-line prompt DNA agent. Start with `agents/master-agent.md` for routing, then load only the specialist agent files needed for the user's task.
...
- If the input is an existing prompt, treat the prompt as the prototype source: extract variables and invariants before rewriting, ask which layers to replace, and offer at least three reconstruction directions unless the user gave a single exact direction.
...
5. Produce deliverables.
   ...
   - D line: first deliver prompt DNA, variable/invariant map, replaceable layers, and three directions; then rewrite the prompt in the selected direction without mentioning the source prompt.
...
- For prompt-to-remake tasks, read `agents/d-line-prompt-dna.md`.
- For internal multi-agent routing, read `agents/master-agent.md`; then load the relevant A0-A11 or D-line specialist file.
