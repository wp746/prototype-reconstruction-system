...
- [ ] 已完成“骨架-骨骼审计”：连续性模式、时间码功能、相机路径、表演动作、空间调度、道具机制、声音边界和终帧状态均已命名。
...
- [ ] B 线长片已先拆成 4-15 秒段落，每段最多 10 个故事板 panel。
- [ ] B 线每段都有 `in_state / action_chain / out_state / continuity_to_next`，且下一段继承上一段 out_state。
...
- [ ] 已选择 Seedance 交付支线：A 线资产直出 / B 线黑白故事板参考 / C 线无资产文本直出 / D 线已有提示词重构；并说明推荐先跑哪条。
- [ ] A 线没有上传故事板，所有身份、场景、道具只由资产图定义。
- [ ] B 线使用的是资产包 + 干净黑白手稿故事板 / 控制帧 + Seedance 2.0 全能参考，不把故事板单独当身份来源。
- [ ] B 线提示词明确全能参考优先级：资产图 > 故事板 > 文字 timing。
- [ ] C 线仅在用户没有资产、没有要求资产/故事板、且明确只要一条 Seedance 提示词时使用；已说明角色/场景/道具一致性弱于 A 线，切点控制弱于 B 线。
- [ ] D 线已完成 `prompt_dna_ledger`、变量/不变量、可替换层、骨架-骨骼审计和至少 3 个重构方向。
- [ ] D 线已确认时间码是动作 beat 还是硬切镜头；一镜到底提示词不得被改成 SH 硬切。
...
- [ ] 所有 errors 已修复。
- [ ] warnings 已确认并写明接受理由。
- [ ] A11 最终评分达到 95/100；低于 95 的提示词没有交付给用户。
